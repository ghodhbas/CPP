    ...
    fc(true)
{
    ...
    fc.setNearPlaneDistance(1.f);
    fc.setFarPlaneDistance(50.f);
...
pcl::PointCloud<pcl::PointXYZ> OcclusionCulling::extractVisibleSurface(UAV camera)
{
    // 1 *****Frustum Culling*******
    ...
    Eigen::Quaternionf quat = camera.get_orientiation();
    Eigen::Vector3f T = camera_pose.block(0, 3, 3, 1);       // The (X, Y, Z) position of the camera w.r.t origin
    ...
    std::vector<pcl::PointXYZ> lineSegments;
    ...
    for (int i = 0; i < (int)output->points.size(); i++)
    {
        ...
        point = pcl::PointXYZRGB(0.f ,0.f ,0.f, 0, 244, 0);
        point.x = centroid[0];
        point.y = centroid[1];
        point.z = centroid[2];
